(function ($, Drupal, drupalSettings) {
  Drupal.behaviors.spc = {
    attach: function (context, settings) {
      ...
      $(".global-search-form #ckan-search")
        .unbind()
        .on("keypress", function (e) {
          if (e.which == 13 || e.which == 124) {
            e.preventDefault();
            window.location.href =
              "https://pacificdata.org/data/dataset?extras_thematic_area_string&q=" +
              $(this).val();
          }
        });
      ...
      if ($("#sdgChart").length && $("#sdgChart svg").length == 0) {
        ...
        var pie = d3
          .pie()
          .value(function (d) {
            return d.value ? d.value : d.barsData.length;
          })
          .sort(null);
        ...
        d3.json(goalsDataPath, function (data) {
          ...
          var goals = svg
            ...
            .attr("aria-label", function (d) {
              return "Go to " + d.data.title;
            })
            .on("mouseover", function (d) {
              goalTooltip.transition().duration(200).style("opacity", 0.9);
              goalTooltip
                .html("Go to " + d.data.title)
                .style("left", d3.event.pageX + "px")
                .style("top", d3.event.pageY - 28 + "px");
            })
            .on("mouseout", function (d) {
              goalTooltip.transition().duration(500).style("opacity", 0);
            });
          ...
          chartBlock
            ...
            .style(
              ...
              "#71a2d6 url(" +
                modulePath +
                "/images/center-image.svg) no-repeat 50% 50%"
            );
          ...
          data.forEach(function (element) {
            element.barsData.forEach(function (item) {
              ...
              item["value"] =
                countriesData["Pacific Regional excl Aust and NZ"][i];
              i += 1;
            });
          });
          ...
          var barScale = d3.scaleLinear().domain([0, 7]).range([0, barHeight]);

          var x = d3.scaleLinear().domain([0, 7]).range([0, -barHeight]);
          ...
          function countRotate(i) {
            ...
            data[i]["rotate"] =
              i === 0
                ? 0
                : data[i - 1].barsData.length * coef + data[i - 1]["rotate"];
            return i === 0
              ? 0
              : data[i - 1].barsData.length * coef + data[i - 1]["rotate"];
          }
          ...
          var arc = d3
            ...
            .startAngle(function (d, i) {
              return (i * 1 * Math.PI) / (numBarsData / 2);
            })
            .endAngle(function (d, i) {
              return ((i + 1) * 1 * Math.PI) / (numBarsData / 2);
            })
            .innerRadius(75)
            .padAngle(0.01);
          ...
          var updateBars = function (data) {
            ...
            var segments = goal.selectAll("path").data(function (d) {
              return d.barsData;
            });
            ...
            segments
              ...
              .style("stroke", function (d) {
                return d.value === 0 ? "#b4b5b4" : "";
              })
              .style("stroke-width", function (d) {
                return d.value === 0 ? "2px" : "";
              })
              .style("stroke-dasharray", function (d) {
                return d.value === 0 ? "2,2" : "";
              });
          };
          ...
          labels
            ...
            .attr("startOffset", function (d, i) {
              return (i * 100) / numBarsData + 50 / numBarsData + "%";
            })
            .text(function (d) {
              return d;
            });
      ...
      if ($("form#ckan-search-form").length) {
        ...
        Form.find("select").on("tokenize:select", function (container) {
          if ($(this).attr("disabled") != "disabled") {
            $(this)
              .tokenize2()
              .trigger("tokenize:search", [$(this).tokenize2().input.val()]);
          }
        });
        ...
        Form.find("select").on("tokenize:dropdown:show", function (e) {
          ...
          $(this)
            .closest(".filter-wrapp")
            .append('<li class="angledown"></li>');
        });
        ...
        Form.find("select").on(
          ...
          function (e, items) {
            ...
            const request_url =
              drupalSettings.spc_home_banner.dataBaseUrl +
              "/data/api/action/package_search?facet.field=";
            let request_params =
              '["organization",+"tags",+"res_format",+"license_id",+"type",+"member_countries",+"topic"]&fq=';
            ...
            const countriesMapping =
              drupalSettings.spc_home_banner.mappingConfig.mapping
                .member_countries;
              ...
              success: function (data) {
                if (data.success) {
                  ...
                  for (let facet_name in search_facets) {
                    ...
                    let facet_select = Form.find(
                      "select[data-name=" + facet_name + "]"
                    );
                    facet_select.empty();

                    if (facet_items.length) {
                      facet_select.removeAttr("disabled");
                      $(".filter-wrapp." + facet_name)
                        .find(".tokens-container")
                      ...
                      for (let option in facet_items) {
                        if (facet_name == "member_countries") {
                          ...
                          if (
                            selected[facet_name] &&
                            selected[facet_name].includes(
                              facet_items[option].name
                            )
                          ) {
                            facet_select.append(
                              $("<option></option>")
  ...
  Drupal.behaviors.homePage = {
    attach: function (context, settings) {
      ...
      $("#nav-popular-datasets-tab").on("click", function () {
        var slide_center = $(
          "#nav-popular-datasets .ckan-dataset-tab-container .carusel-of-items"
        )
          .find(".slick-center")
          .first();
      ...
      if (
        $(".latest-stories-slider").length > 0 &&
        $(".latest-stories-slider .slick-dots li").length > 0
      ) {
        let slides_num_stories = $(".latest-stories-slider .slick-dots li")
          .length;
        let slide = $(
          ".latest-stories-slider .slick-dots .slick-active button"
        ).text();
        if ($(".latest-stories-slider .slide-number").length == 0) {
          $(".latest-stories-slider", context).append(
            '<div class="slide-number"><strong>' +
              slide +
              "</strong> of <strong>" +
              slides_num_stories +
              "</strong></div>"
        ...
        $(".latest-stories-slider .slick-arrow").on("click", function () {
          ...
          $(".latest-stories-slider .slide-number").html(
            "<strong>" +
              slide +
              "</strong> of <strong>" +
              slides_num_stories +
      ...
      if (
        $(".data-insights-list").length > 0 &&
        $(".data-insights-list .slick-dots li").length > 0
      ) {
        let slides_num_di = $(".data-insights-list .slick-dots li").length;
        let slide = $(
          ".data-insights-list .slick-dots .slick-active button"
        ).text();
        if ($(".slide-number").length == 0) {
          $(".data-insights-list").append(
            '<div class="slide-number"><strong>' +
              slide +
              "</strong> of <strong>" +
              slides_num_di +
        ...
        $(".related-data-insights .slick-arrow").on("click", function () {
          slide = $(
            ".related-data-insights .slick-dots .slick-active button"
          ).text();
          $(".related-data-insights .slide-number").html(
            "<strong>" +
              slide +
              "</strong> of <strong>" +
              slides_num_di +
    ...
    attach: function (context, settings) {
      ...
      var ds_steps = [
        ...
        },
        {
          ...
          position: "center-left",
          content:
            "If you still canâ€™t find the dataset you want in the list, make use of our â€œSuggest a datasetâ€ form.",
          buttons: [
            { text: "Prev", className: "tour-prev-btn", click: prevStep },
